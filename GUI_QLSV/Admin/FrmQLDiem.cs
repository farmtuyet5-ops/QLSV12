...
namespace GUI_QLSV.Admin
{
    public partial class FrmQLDiem : Form
    {
        ...
        {
            InitializeComponent();
            dgvDSDiem.CellClick += dgvDSDiem_CellClick;
        ...
        private decimal? LayDiemNullable(TextBox txt)
        {
            if (string.IsNullOrWhiteSpace(txt.Text))
                return null;

            decimal diem;
            if (decimal.TryParse(txt.Text.Trim(), out diem))
                return diem;

            return -1;
        }
        private DTO_BangDiem LayThongTin()
        {
            decimal? diemGK = LayDiemNullable(txtDiemGK);
            decimal? diemCK = LayDiemNullable(txtDiemCK);
